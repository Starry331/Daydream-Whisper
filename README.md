...
- Supports named profiles for repeated dictation, meeting, subtitle, or domain-specific workflows
...
JSON requests can also target a local file path and apply a profile:
...
The server keeps a persistent transcription worker per loaded model, supports CORS for local browser-based apps, returns `X-Request-ID` and timing headers, and exposes `/metrics` for lightweight monitoring.
...
- a local app/bundle directory that contains an embedded Whisper subdirectory such as `speech_encoder/` or `whisper/`
...
These files are optional. You can still override everything directly from the CLI or API request body.
...
- Passing a local bundle root works as long as it contains an embedded Whisper runtime directory such as `whisper/` or `speech_encoder/`.
...
`dwhisper serve` 现在会暴露一个面向语音转文字应用的本地 HTTP API，包含 `/health`、`/ready`、`/metrics`、`/v1/audio/transcriptions`、`/v1/audio/translations` 和 `/v1/models`。服务端会为每个已加载模型保留常驻转录 worker，减少外部听写应用重复请求时的启动抖动；如果你希望首个请求也尽量快，可以加 `--preload`。它还会返回 `X-Request-ID`、处理时长和并发相关 headers，并支持 `profile`、`hotwords`、`vocabulary`、`correction` 这些请求字段。

配置文件在 `~/.dwhisper/config.yaml`，纠错规则可以放在 `~/.dwhisper/corrections.yaml`，词表可以放在 `~/.dwhisper/vocabulary.yaml`，命名 profile 可以放在 `~/.dwhisper/profiles.yaml`。本地模型除了标准 Whisper 目录，也支持传入带有 `whisper/` 或 `speech_encoder/` 子目录的 bundle 根目录。所有核心选项都支持 `DWHISPER_*` 环境变量覆盖，而且安装脚本只会创建 `dwhisper`，不会覆盖你原来的 `daydream` 命令。
